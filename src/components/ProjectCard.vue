...
<template><q-card class="row q-ma-sm" :style="$q.screen.gt.sm ? 'width: 550px' : 'width: 95%'" style="min-height: 250px" bordered>
  <q-card-section class="col">
    <q-card-section :class="$q.screen.gt.sm ? 'q-pt-xs' : 'q-px-none'">
      <div class="text-h5 q-mt-sm q-mb-xs">
        {{ title }}
        <q-badge class="q-pa-xs" :color="status.color" align="top">{{ status.label }}
        </q-badge>
      </div>
  ...
  <q-card-actions vertical class="justify-around" :class="$q.screen.gt.sm ? 'q-px-md' : 'q-px-sm'">
    ...
    <q-dialog v-model="techStack" class="q-pa-none q-ma-none">
      <q-card class="my-card q-pa-none q-ma-none">
        <q-card-section class="q-pa-none q-ma-none">
          <div class="row">
            <component v-for="(tile, index) in tech.filter(t => tileMap[t])" :key="index" :is="tileMap[tile]" />
          </div>
          <q-page-sticky position="top" :offset="$q.screen.gt.sm ? [0, 140] : [0, -40]">
            <q-btn fab icon="close" color="primary" v-close-popup />
          </q-page-sticky>
        </q-card-section>
      </q-card>
    </q-dialog>
